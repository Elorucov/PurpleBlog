...
```
<p><a href="{{relativeUrl}}">{{title}}</a> <span>{{published}}</span><div class="summary">{{summary}}</div></p>
```

Here, {{relativeUrl}} is a URL to the post. The date in the {{publised}} tag is specified in the M/D format.

```
<p><a href="how-to-make-a-thing/">How to make a thing</a> <span>1/20</span><div class="summary">A guide on how to make a thing</div></p>
```